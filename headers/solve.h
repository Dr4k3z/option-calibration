...
public:
       ...
       void set_initial_condition(float (*f)(float)); // function pointer ?????
...
private:
       ForwardEulerMethod(const AdvDiffReactEquation* eqt) : FiniteDifferenceMethod(eqt) { std::cout << "ForwardEulerMethod constructor called\n"; }
       friend class PDESolver;
public:
       Solution solve() const override { std::cout << "forward euler called\n"; }
};
